...
#pragma unroll
   for(size_t color=0; color < 3; ++color) {
     ...
     A_add_sign_iB<FType,isign>(spinor_out(color,0), t,t2 );
...
#pragma unroll
   for(size_t color=0; color < 3; ++color) {
     ...
     A_add_sign_iB<FType,isign>(spinor_out(color,1),t,t2);
   }
 }
 ...
 template<typename T, typename VN,  typename T2, int isign,
 	...
 	sycl::access::target accessTarget = sycl::access::target::global_buffer>
 inline
 void SyCLProjectDir0Perm( const  SyCLSGVSpinorViewAccessor<T,VN,accessMode,accessTarget>& in,
		       HalfSpinorSiteView<T2>& spinor_out,
		       const size_t& i,
		       const sycl::intel::sub_group& sg)
 {
...
inline
   ...
   for(size_t color=0; color < 3; ++color) {
       ...
       MGComplex<FType> t =  Load( in.offset(i,0,color,0,0), in.get_pointer(), sg);
       MGComplex<FType> t2=  Load( in.offset(i,3,color,0,0), in.get_pointer(), sg);
       A_add_sign_B<FType,-isign>(spinor_out(color,0), t,t2);
     }
...
#pragma unroll
     for(size_t color=0; color < 3; ++color) {
       ...
       MGComplex<FType> t =  Load( in.offset(i,1,color,0,0), in.get_pointer(), sg);
       MGComplex<FType> t2=  Load( in.offset(i,2,color,0,0), in.get_pointer(), sg);
       A_add_sign_B<FType,+isign>(spinor_out(color,1), t,t2);
 ...
 template<typename T, typename VN, typename T2, int isign,
 	...
 	sycl::access::target accessTarget = sycl::access::target::global_buffer>
inline
   void SyCLProjectDir1Perm(const SyCLSGVSpinorViewAccessor<T, VN,accessMode,accessTarget>& in,
			HalfSpinorSiteView<T2>& spinor_out,
			const size_t i,
			const sycl::intel::sub_group& sg)
   {
...
#pragma unroll
 ...
 template<typename T, typename VN, typename T2, int isign,
   ...
   sycl::access::target accessTarget = sycl::access::target::global_buffer>
   inline
 void SyCLProjectDir2Perm(const SyCLSGVSpinorViewAccessor<T, VN, accessMode, accessTarget>& in,
		      HalfSpinorSiteView<T2>& spinor_out,
		      const size_t i,
		      const sycl::intel::sub_group& sg)
 {
...
#pragma unroll
 ...
 template<typename T, typename VN, typename T2, int isign,
 	...
 	sycl::access::target accessTarget = sycl::access::target::global_buffer>
inline
void SyCLProjectDir3Perm(const SyCLSGVSpinorViewAccessor<T, VN, accessMode>& in,
		     HalfSpinorSiteView<T2>& spinor_out,
		     const size_t i,
		     const sycl::intel::sub_group& sg)
{
